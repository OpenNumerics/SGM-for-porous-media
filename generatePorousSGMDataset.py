...
def worker(n, x_cells, l_values, U0_values, F_right_values,
        ...
        good = not ((pt.isnan(c_T).any() or pt.isnan(phi_T).any() or
                     pt.isinf(c_T).any() or pt.isinf(phi_T).any()))
        
        return n, good, c_T, phi_T
...
if __name__ == '__main__':
    ...
    max_workers = os.cpu_count()  # or set explicitly
    with ProcessPoolExecutor(max_workers=max_workers) as ex:
        futures = [
            ex.submit(worker, n, x_cells_cpu, l_cpu, U0_cpu, F_cpu,
                    eps_min, eps_max, kappa0, D0, b, t_plus, a_s, k_rn, c0, c_right,
                    T, dt, G)
    ...
    pt.save(pt.Tensor(gp_success), './data/gp_success.pt')
